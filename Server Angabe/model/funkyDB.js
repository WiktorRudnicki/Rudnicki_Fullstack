...
async function getCarsDB() {
  const result = await db.query('SELECT cars.id as id, title, status, price, image, miles, year_of_make, description, first_name, last_name from cars join owner on owner = owner.id');
  return result.rows;
}
